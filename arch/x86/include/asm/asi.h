 ...
 *
 * Setup: First call asi_init_domain() to create a domain. At present only one
 * domain can be created per mm per class, but it's safe to asi_init_domain()
 * this domain multiple times. For each asi_init_domain() call you must call
 * asi_destroy() AFTER you are certain all CPUs have exited the nonsensitive
 * address space (by calling asi_exit()).
 *
 * Runtime usage:
 *
 * 1. Call asi_enter() to switch to the nonsensitive address space. This can't be
 ...
 *       switches to the sensitive address space.
 *
 ...
 * from and/or to the nonsensitive address space, and to return to userspace
 * directly from the nonsensitive address space, or _in_ it.
 *
 ...
 * nonsensitive address space before returning from the outermost
 * exception/interrupt.
 *
 * Note: ASI does not modify KPTI behaviour; when ASI and KPTI run together
 * there are 2+N address spaces per task: the sensitive kernel address space,
 * the user address space, and one nonsensitive (kernel) address space for each of
 * the N ASI classes.
 */
...
/*
 * An ASI domain (struct asi) represents a nonsensitive address space. The
 * sensitive address space (and user address space under PTI) are not
...
/* Immediately exit the nonsensitive address space if in it */
void asi_exit(void);
...
{
	return asi_enabled_static()
	       ? this_cpu_read(curr_asi)
	       : NULL;
}

/* Are we currently in a nonsensitive address space? */
static __always_inline bool asi_in_nonsensitive(void)
{
...
{
	if (asi_enabled_static() && asi_is_tense()) {
