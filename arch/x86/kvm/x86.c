...
static inline int kvm_x86_init_asi_class(void)
{
	 ...
	 *
	 *
	 * If we decided to map userspace into the guest's nonsensitive address
...
int kvm_arch_vcpu_ioctl_run(struct kvm_vcpu *vcpu)
{
	 ...
	 * from the nonsensitive address space to the user address space. So we
	 * just return to the sensitive address space in between.
	 */
	asi_exit();
...
int kvm_arch_init_vm(struct kvm *kvm, unsigned long type)
{
	...
	ret = asi_init_domain(kvm->mm, ASI_CLASS_KVM, &kvm->arch.asi);
	if (ret)
		goto out_uninit_mmu;
