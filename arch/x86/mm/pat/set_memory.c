...
/*
 * Having updated the sensitive PGD, reflect this change in the ASI
 * nonsensitive address space too.
 */
static inline int mirror_asi_direct_map(struct cpa_data *cpa, int primary)
{
	...
	/* TODO: Comment about the fact that it's fine to allocate here. */

	/* Only need to do this for the real sensitive direct map. */
	...
	asi_cpa.curpage = 0;
	return __change_page_attr(&asi_cpa, primary);
}
 ...
 */
int set_direct_map_sensitive(struct page *page, unsigned long num_pageblocks, bool sensitive)
{
	...
	if (WARN_ON_ONCE(!asi_nonsensitive_pgd))
		return 0;
...
/*
 * Walk the pagetable to check if the page is mapped into all ASI nonsensitive
 * address spaces.
 */
