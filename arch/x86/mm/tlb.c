...
{
	BUILD_BUG_ON(X86_CR3_ASI_PCID_BITS < order_base_2(ASI_MAX_NUM_CLASSES));
...
noinstr unsigned long __get_current_cr3_fast(void)
{
	...
	/*
	 * Outside of the ASI critical section, an ASI-nonsensitive CR3 is
...
static inline bool cr3_matches_current_mm(void)
{
	...
	barrier();
	pgd_cr3 = read_cr3_pa();
	return pgd_cr3 == __pa(current->mm->pgd) || pgd_cr3 == pgd_asi;
}
