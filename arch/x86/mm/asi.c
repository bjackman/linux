...
static int asi_suspend(void)
{
	 ...
	 * in the kexec path). Saving a nonsensitive CR3 and restoring it later
...
static void __asi_init_user_pgds(struct mm_struct *mm, struct asi *asi)
{
	 ...
	 * sensitive pgd to the nonsensitive pgd.
	 */
...
/*
 * This function is to copy the given sensitive pgd entry for
 * userspace addresses to the corresponding nonsensitive pgd entries.
 * It means that the sensitive pgd entry must be updated before
 * this function is called.
 * We map entire userspace addresses to the nonsensitive address spaces
 * by copying sensitive pgd entries to the nonsensitive page tables
 * so that we don't need to maintain consistency of lower level PTEs
 * between the sensitive page table and the nonsensitive page tables.
 */
void asi_clone_user_pgtbl(struct mm_struct *mm, pgd_t *pgdp)
{
	 ...
	 * the owner mm for this pgd in this case.
	 */
	if (!mm) {
	...
	for (i = 0; i < ARRAY_SIZE(mm->asi); i++) {
		 ...
		 * the sensitive pgd and updating the corresponding
