...
alloc:
	 ...
	 */
	keys = kvmalloc_array(key_size, bucket_size, BPF_GFP_USER_VMALLOC | __GFP_NOWARN);
	values = kvmalloc_array(value_size, bucket_size, BPF_GFP_USER_VMALLOC | __GFP_NOWARN);
	if (!keys || !values) {
		ret = -ENOMEM;
		goto after_loop;
	}
